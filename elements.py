...
from ableton.v3.control_surface import MIDI_NOTE_TYPE, ElementsBase, MapMode, \
    create_button, create_sysex_sending_button
...
def create_rgb_button(identifier, name=None, **k):
    return create_sysex_sending_button(identifier, name,
        midi.LED_HEADER + (midi.BUTTON_ID_TO_SYSEX_ID[identifier],),
        is_rgb=True, **k)
...
class Elements(ElementsBase):
    ...
    def __init__(self, *a, **k):
        ...
        if TAP_SHIFT_MODE:
            self.add_modified_control(control=(self.rewind_button),
                                      modifier=(self.tap_button),
                                      name="previous_device_button")
            self.add_modified_control(control=(self.fastforward_button),
                                      modifier=(self.tap_button),
        ...
        if PY_SAVE_PROJECT:
            self.add_modified_control(control=(self.save_button),
                                      modifier=(self.tap_button),
                                      name="save_project_button")
        ...
        if PY_TOGGLE_WRENCH:
            self.add_modified_control(control=(self.part_button),
                                      modifier=(self.tap_button),
                                      name="wrench_toggle_button")
        ...
        if TAP_SHIFT_MODE and TAP_PADS_MUTE_SOLO:
            ...
            row = [self.pad_bank_b._orig_buttons[0][i] for i in
                   [4, 5, 6, 7, 0, 1, 2, 3]]
        ...
        self.add_submatrix((self.continuous_controls), "Encoders",
                           columns=(0, 8))
        self.add_submatrix((self.continuous_controls), "Faders",
                           columns=(8, 16))
        ...
        self.add_sysex_element((midi.DISPLAY_HEADER + (1,)),
                               ...
                               send_message_generator=(
                                   lambda text: midi.make_full_screen_message(1,
                                                                              (
                                                                              0,),
                                                                              text)),
                               optimized=True)
        self.add_sysex_element((midi.DISPLAY_HEADER + (3,)),
                               ...
                               send_message_generator=(
                                   lambda icon1, icon2, icon3,
                                       icon4: midi.DISPLAY_HEADER + (3,) + (
                                   16, icon1.state.value, 0, 18,
                                   icon1.type.value, 0) + (
                                              32, icon2.state.value, 0, 34,
                                              icon2.type.value, 0) + (
                                              48, icon3.state.value, 0, 50,
                                              icon3.type.value, 0) + (
                                              64, icon4.state.value, 0, 66,
                                              icon4.type.value, 0) + (
                                              midi.SYSEX_END,)), optimized=True)
        self.add_sysex_element((midi.DISPLAY_HEADER + (23,)),
                               "Display_Popup_Command", send_message_generator=(
                partial(midi.make_full_screen_message, 23)), optimized=True)
        self.display_parameter_commands = []
        for (i, element) in enumerate(
            self.continuous_controls_raw + [self.encoder_9, self.fader_9]):
            self.add_sysex_element((midi.DISPLAY_HEADER + (32, i)),
                                   ("Display_Parameter_Command_{}".format(i)),
                                   send_message_generator=partial(
                                       (midi.make_full_screen_message),
                                       line3=(element.original_identifier())),
                                   optimized=True)
            self.display_parameter_commands.append(
                getattr(self, "display_parameter_command_{}".format(i)))
